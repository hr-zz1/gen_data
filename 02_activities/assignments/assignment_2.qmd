...
```{r}
hist.data <- fread("./02_activities/data/gwa.A2.fam")
...
Yes, the data looks roughly Gaussian
...
```{bash}
plink2 --bfile ./02_activities/data/gwa.qc_unrelated.A2 --extract ./02_activities/data/gwa.qc.A2_pruned.prune.in --make-bed --out ./02_activities/data/gwa.qc.A2_unrelated_pruned
...
# loading raw PLINK linear data
assoc.A2 <- data.table::fread("./02_activities/data/gwa.qc.A2_assoc.PHENO1.glm.linear")

# Keep only the additive test (one row per SNP)
assoc.A2_add <- assoc.A2[TEST == "ADD"]

# change PLINK terms #CHROM, POS, ID to qqman terms CHR, BP, SNP,
